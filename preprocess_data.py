...
class NpzParser:
    ...
    def __init__(self, data_dir, circuit_path, label_path, random_shuffle=True, trainval_split=None):
    ...
    class inmemory_dataset(InMemoryDataset):
        ...
        def process(self):
            ...
            for cir_idx, cir_name in enumerate(circuits):
                ...
                graph = parse_pyg_mlpgate(x, edge_index, tt_dis, min_tt_dis, tt_pair_index, prob, rc_pair_index, is_rc, edge_index_s)
                graph.name = cir_name
    ...
    with open(bench_file, 'r') as file:
        ...
        for line in lines:
            ...
            if line.startswith('INPUT'):
                input_nodes.append(int(re.search('\\((\\d+)\\)', line).group(1)))
            elif line.startswith('OUTPUT'):
                output_nodes.append(int(re.search('\\((\\d+)\\)', line).group(1)))
            elif '=' in line and 'AND' in line:
                left, right = line.split('=')
                output_id = int(left.strip())
                inputs = re.findall('\\((\\d+), (\\d+)\\)', right)
                ...
                left, right = line.split('=')
                output_id = int(left.strip())
                input_id = int(re.search('\\((\\d+)\\)', right).group(1))
                edges.append([input_id, output_id, -1])
                not_nodes.append(output_id)
    signed_edge = pd.DataFrame(edges, columns=['Input', 'Output', 'NOT_Flag'])
    id_map = {'input_nodes': input_nodes, 'output_nodes': output_nodes, 'and_nodes': and_nodes, 'not_nodes': not_nodes}
...
def load_raw_data_and_transform(root_dir, circuit_path, label_path, bench_path):
    ...
    with Progress() as progress:
        ...
        for cir_idx, cir_name in enumerate(circuits):
            ...
            _, signed_edge = parse_bench_v2(bench_file)
...
def generate_pi_edges(root_dir, circuit_path, label_path, bench_path):
    ...
    for cir_idx in track(range(len(dataset))):
        ...
        sparse_adj = torch.sparse.FloatTensor(indices=torch.stack([rows, cols]), values=weights, size=(node_num, node_num))
...
def save_split_data(root_dir, circuit_path, label_path, split_ratio):
    dataset = NpzParser(root_dir.joinpath('npz'), circuit_path, label_path, random_shuffle=True, trainval_split=split_ratio)
    train_dataset, val_dataset, test_dataset = dataset.get_dataset()
    train_valid_str = f'{split_ratio[0]}-{split_ratio[1]}-{split_ratio[2]}'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Preprocess BiSeqGate AIG datasets.')
    parser.add_argument('--base_dir', default=os.environ.get('BISEQGATE_BASE_DIR', '.'), help='Project directory containing AIGDataset/. Defaults to BISEQGATE_BASE_DIR or the current directory.')
    parser.add_argument('--split_ratio', nargs=3, type=float, default=[0.01, 0.01, 0.98], metavar=('TRAIN', 'VALID', 'TEST'), help='Train/valid/test split ratio.')
    args = parser.parse_args()
    base_dir = Path(args.base_dir).expanduser().resolve()
    root_dir = Path(base_dir) / 'AIGDataset/PolarGate_processed'
    circuit_path = Path(base_dir) / 'AIGDataset/PolarGate_raw/npz/graphs.npz'
    label_path = Path(base_dir) / 'AIGDataset/PolarGate_raw/npz/labels.npz'
    bench_path = Path(base_dir) / 'AIGDataset/PolarGate_raw/bench'
    ...
    save_split_data(root_dir, circuit_path, label_path, split_ratio)
